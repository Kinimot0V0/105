...
const submitForm = async () => {
  ...
  try {
    ...
    const turbineIdValue = parseInt(formData.turbineId);
    if (!isNaN(turbineIdValue)) {
      pointData.turbineId = turbineIdValue;
    }
  ...
  <div class="form-container">
    ...
    <el-form ref="formRef" :model="formData" :rules="rules" label-width="120px">
      ...
      <el-row :gutter="20" style="margin-top: 10px">
        ...
        <!-- 风机 -->
        <el-col :span="8">
          <el-form-item label="风机ID" prop="turbineId">
            <el-input 
              v-model="formData.turbineId" 
              placeholder="请输入风机ID(整数)"
              @input="formData.turbineId = ($event || '').toString().replace(/[^\d]/g, '')"
            />
          </el-form-item>
        </el-col>
      </el-row>
