...
import { getWindFarmList,getAlg,getTurbineList,getModuleList } from '@/api/model'
// 从父组件传递的 props 数据
...
//初始化风机
const initTurbine = async () => {
  try {
    const response = await getTurbineList()
    
    // 添加组合名称逻辑
    wind_turbine.value = response.data.result.map(turbine => {
      const matchedFarm = windFarmList.value.find(
        farm => farm.windFarmId === turbine.windFarmId
      )
      return {
        ...turbine,
        combinedName: matchedFarm 
          ? `${matchedFarm.windFarmName}${turbine.turbineName}`
          : turbine.turbineName
      }
    })
