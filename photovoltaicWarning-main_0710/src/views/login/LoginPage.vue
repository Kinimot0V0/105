...
const getCompanyList = async () => {
  try {
    ...
    if (companyList.value.length > 0) {
      // 默认选择第一个公司
      companyId.value = companyList.value[0].companyId
    }
    return response
  } catch (error) {
...
const confirmCompanySelection = async () => {
  ...
  try {
    ...
    ElMessage.success('公司选择成功')
  } catch (error) {
    console.error('公司ID保存失败:', error)
    ElMessage.error('公司选择失败，请重试')
  }
}
  ...
  <el-row class="login-page">
    <el-col class="form">
      <el-form :model="loginForm" class="login-form">
        <h1 class="title">光伏智能监测与故障预警系统</h1>
  ...
  >
    <el-form>
      <el-form-item label="公司" label-width="80px">
        <el-select v-model="companyId" placeholder="请选择公司" style="width: 100%">
          <el-option
    ...
    <template #footer>
      <el-button type="primary" @click="confirmCompanySelection">确定</el-button>
    </template>
  </el-dialog>
</template>
