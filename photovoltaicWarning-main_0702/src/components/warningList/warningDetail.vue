...
const getPointInfo = async () => {
  try {
    ...
    const paramsFarm = { deviceId: props.deviceId }
    ...
    console.error('获取逆变器测点数据失败:', error)
  }
}
...
<template>
  <div class="container1">
    ...
    <div class="info-text">
      ...
      <span class="span-value" style="width: 220px">{{ pvFarmName }}-{{ props.deviceName }}</span>
        ...
        <div class="otherChart">
          ...
          <template v-if="currentPicType === 3">
            <div v-if="thresholdPicData.length" class="trend-scroll">
              <!-- 一张图占一行，超出就滚动 -->
              <div v-for="(chart, idx) in thresholdPicData" :key="idx" class="trend-wrapper">
                <div class="chart-title">{{ chart.title }}</div>
