...
const handleClose = () => {
  ...
  if (selectedRows.length > 0) {
    const hasInvalidStatus = selectedRows.some(row => row.warningStatus == 4)
    if (hasInvalidStatus) {
      ElMessage.warning('请不要选择已关闭的预警信息')
      return
    }
...
const confirmClose = () => {
  ...
  const hasInvalidStatus = selectedRows.some(row => row.warningStatus !== 3)
  if (hasInvalidStatus) {
    ElMessage.warning('请仅选择关闭待确认的预警信息')
    return
  }
  ...
  <div class="container">
    ...
    <div class="main">
      <div class="info">
        ...
        <div class="info-button">
          ...
          <el-button class="operation" :disabled="selectedExpandedRows.length === 0" @click="handleLevel"
            >分级</el-button
          >
          <el-button class="operation" :disabled="selectedExpandedRows.length === 0" @click="confirmClose"
            >确认关闭</el-button
          >
        </div>
      </div>
      ...
      <el-table :data="warningList" :key="tableKey" row-key="warningDescription" height="70vh">
        ...
        <el-table-column prop="warningStatus" label="状态" width="140px" align="center">
          <template #default="scope">
            <span :style="{ color: statusMap[scope.row.warningStatus]?.color || 'black' }">
