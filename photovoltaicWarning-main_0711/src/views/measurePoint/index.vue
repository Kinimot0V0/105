...
const getRealPointData = async () => {
  try {
    ...
    realPointList.value = data.realPointList

    // // [修改点] 过滤掉没有 deviceId 的条目，并提取唯一值
    // const deviceIds = [
    //   ...new Set(
    //     points.map((point) => point.deviceId).filter((id) => id != null) // 过滤 null/undefined
    //   )
    // ]

    // const uncachedIds = deviceIds.filter((id) => !deviceCache.value[id])

    // if (uncachedIds.length > 0) {
    //   const devicePromises = uncachedIds.map((id) =>
    //     getFarmInfo({ deviceId: id })
    ...
    //   deviceResults.forEach((info) => {
    //     if (info) {
    //       deviceCache.value[info.deviceId] = {
    ...
    // // [修改点] 合并数据时处理可能缺失 deviceId 的情况
    // realPointList.value = points.map((point) => ({
    //   ...point,
    //   ...(point.deviceId ? deviceCache.value[point.deviceId] : {}) // 安全解构
    // }))
...
<template>
  <div class="container">
    ...
    <el-table :data="realPointList" border>
      ...
      <!-- <el-table-column prop="deviceName" label="逆变器名称" align="center"></el-table-column>
