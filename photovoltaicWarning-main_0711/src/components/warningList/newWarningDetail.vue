...
import { getDeviceInfo, getTrendData, getFarmInfo, showPictures } from '@/api/warningDetail'
...
//     const paramsFarm = { deviceId: props.deviceId, deviceType: props.modelType }
//     const responseFarm = await getFarmInfo(paramsFarm)
//     const resultFarm = responseFarm.data.result
//     pvFarmName.value = resultFarm.pvFarmName

//     // 清空原有数据并构建新数据结构
//     treeData.splice(0, treeData.length)
//     treeData.push({
//       id: `${props.deviceId}`,
//       label: `${pvFarmName.value}-${props.deviceName}`,
//       children: result.map((point) => ({
//         // 直接使用 pointId 作为唯一标识
//         id: `${point.standPointId}`,
//         label: point.pointDescription,
//         pointLabel: point.pointLabel, // 保留原始标签
//         pointType: point.pointType, // 保留测点类型
//         used: point.used
//       }))
//     })

//     // 计算默认选中测点
//     const allUsedPoints = result.filter((point) => point.used === 1)
//     const selectedCount = Math.max(1, Math.ceil(allUsedPoints.length / 3))
//     const selectedPointsList = allUsedPoints.slice(0, selectedCount)

//     // 设置默认选中和展开的keys
//     defaultCheckedKeys.value = selectedPointsList.map((p) => `${p.standPointId}`)
//     defaultExpandedKeys.value = [`${props.deviceId}`] // 始终展开设备节点

//     // 更新选中测点列表
//     selectedPoints.value = selectedPointsList.map((point) => ({
//       id: `${point.standPointId}`,
//       name: point.pointDescription
//     }))
...
//     })
//   } catch (error) {
//     console.error('获取设备测点数据失败:', error)
//   }
// }
...
const getPointInfo = async () => {
  try {
    ...
    const yellowPoints = []
    treeData[0].children.forEach(pt => {
      if (pt.used === 1 && yellowPoints.length < 2) {
        yellowPoints.push({ id: pt.id, name: pt.label })
      }
    })
...
const getCompareScatter = async () => {
  try {
    ...
    const pointIdList = selectedComparePoints.value.map(p => parseInt(p.id))

    const params = {
      deviceId: props.deviceId,
      deviceType: props.modelType,
      pointIdList,
    ...
    // 建立按 pointId 的映射，兼容返回顺序不一致
    const resultByPointId = new Map()
    result.forEach(item => {
      const pid = item.pointId ?? item.standPointId ?? null
      resultByPointId.set(pid, item)
    })
...
const getTrend = async () => {
  try {
    // 收集选中的测点id（需解析组合ID）
    const pointIdList = selectedPoints.value.map((p) => parseInt(p.id))
    ...
    // console.log('getTrendData:', params)
    const response = await getTrendData(params)
    // console.log('getTrendData返回:', response)
    const result = response.data.result || []
    ...
    switch (currentPicType.value) {
      case 1: // ===== 散点图 =====
        scatterData.value = result.map((item) => {
          ...
          const map = new Map()
          xPoint.pointValue.forEach((x) => map.set(x.datetime, { x: x.value, datetime: x.datetime }))
          ...
          return {
            ...
            data: Array.from(map.values())
              ...
              .map((d) => {
                const isAlarm =
                  new Date(d.datetime) >= new Date(props.startTime) && new Date(d.datetime) <= new Date(props.endTime)
                return {
      ...
      case 2: // ===== 趋势图（可能是带阈值的或普通趋势图）=====
        result.forEach((pic) => {
          const chartData = {
            title: pic.picName,
            lines: pic.points.map((pt) => ({
              name: pt.pointDescription,
              data: pt.pointValue.map((pv) => [new Date(pv.datetime).getTime(), Number(pv.value.toFixed(2))])
            }))
          }

          // 判断是否有阈值
          if (pic.threshold !== undefined && pic.threshold !== null) {
            // 有阈值添加到阈值图表数据
            thresholdPicData.value.push({
              ...chartData,
              threshold: Number(pic.threshold.toFixed(2))
            })
          } else {
            // 没有阈值添加到普通趋势图表数据
            trendPicData.value.push(chartData)
          }
        })
        break
    }
        ...
        <div class="summary-section">
          ...
          <div class="info-grid">
            <el-descriptions :column="2" border >
              ...
              <el-descriptions-item label="预警等级"><el-tag type="danger">{{ levelMap[props.warningLevel]?.label}}</el-tag></el-descriptions-item>
              <el-descriptions-item label="预警状态"><el-tag type="danger">{{ statusMap[props.warningStatus]?.label}}</el-tag></el-descriptions-item>
              <el-descriptions-item label="预警管理等级">模板：二类缺陷（4级）</el-descriptions-item>
              <el-descriptions-item label="预警编码标准分类">模板：MDY05(电气传动链-变流器控制及附属系统-电网)</el-descriptions-item>
              ...
              <el-descriptions-item label="关键测点">模板：‘风速’、‘有功功率’</el-descriptions-item>
              <el-descriptions-item label="预警原因">模板：风速-功率数据点偏离正常区间范围，SCADA控制导致的降容</el-descriptions-item>
            </el-descriptions>
            <!-- <div class="info-row">
              <span class="info-label">预警设备</span>
              <span class="info-value">{{ props.deviceName }}</span>
            </div>
            <div class="info-row">
