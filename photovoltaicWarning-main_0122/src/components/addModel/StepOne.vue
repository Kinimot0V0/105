...
const rules = {
  ...
  turbineList: [{ required: true, type: 'array', min: 1, message: '至少选择一个光伏机器', trigger: 'change' }],
  alertInterval: [{ required: true, validator: validateInteger, message: '预警间隔必须是整数', trigger: 'blur' }],
  alert_window_size: [
    { required: true, validator: validateInteger, message: '预警数据长度必须是整数', trigger: 'blur' }
  ],
...
const turbineList = ref([]) // 当前选择的风场对应的光伏机器列表
...
const onTurbineSelect = (selectedTurbineId) => {
  ...
  if (selectedTurbine && !localData.turbineList.some((t) => t.turbineId === selectedTurbine.turbineId)) {
    // 如果光伏机器对象存在且不在已选光伏机器列表中，则添加到 localData.turbineList
...
const removeTurbine = (turbineId) => {
  // 直接从 localData.turbineList 中删除指定的光伏机器
  ...
  <div class="model-config-container">
    ...
    <div class="basic-info">
      <el-form :model="localData" :rules="rules" ref="stepForm" label-width="120px">
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="已选择光伏机器" prop="turbineList">
        ...
        <el-row :gutter="20">
          ...
          <!-- 预警数据长度 -->
          <el-col :span="6">
            <el-form-item label="预警数据长度" prop="alert_window_size">
              <el-input v-model="localData.alert_window_size" placeholder="请输入预警数据长度（整数）"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
