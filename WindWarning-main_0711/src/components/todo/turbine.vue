  ...
  props.treeData.forEach((farm) => {
    ...
    if (farm.children && farm.children.length > 0) {
      farm.children.forEach((turbine) => {
        // 计算风机的新总数
        const turbineWarningLevel1Sum =
          (turbine.warningLevel1waitCloseWaitSum ?? 0) +
          (turbine.warningLevel1waitDoneSum ?? 0) +
          (turbine.warningLevel1waitHangUpSum ?? 0)

        const turbineWarningLevel2Sum =
          (turbine.warningLevel2waitCloseWaitSum ?? 0) +
          (turbine.warningLevel2waitDoneSum ?? 0) +
          (turbine.warningLevel2waitHangUpSum ?? 0)

        const turbineTotal = (turbine.reportSum ?? 0) + turbineWarningLevel1Sum + turbineWarningLevel2Sum

        turbineTotalCounts[`${farm.windFarm_id}_${turbine.turbine_id}`] = turbineTotal

        // 累加风场总数
        farmReportSum += turbine.reportSum ?? 0
        farmWarningLevel1Sum += turbineWarningLevel1Sum
        farmWarningLevel2Sum += turbineWarningLevel2Sum
      })
    }
      ...
      <li v-for="(node, index) in props.treeData" :key="index" class="tree-node">
        <div class="tree-label">
          ...
          <span class="label"
            ...
            <!-- 风场节点右上角的数量标签 -->
            <span v-if="calculateTotalCount.farmTotalCounts[node.windFarm_id] !== undefined" class="count-badge">
