...
import { addRealPoint, getModuleList, getRealPoint } from '@/api/realPoint'
...
const validateCalculate = (rule, value, callback) => {
  ...
  if (['+', '-', '*', '/'].includes(parts[0])) 
    return callback(new Error('表达式不能以运算符开头'))
  if (['+', '-', '*', '/'].includes(parts[parts.length-1])) 
    return callback(new Error('表达式不能以运算符结尾'))
...
const addLabel = () => {
  ...
  if (expressionParts.value.length % 2 !== 0) 
    return ElMessage.error('需要先添加运算符')
...
const addOperator = () => {
  ...
  if (expressionParts.value.length % 2 === 0) 
    return ElMessage.error('需要先添加测点标签')
    ...
    <el-form ref="formRef" :model="formData" :rules="rules" label-width="120px">
      ...
      <el-form-item label="计算测点配置" prop="calculate">
        <el-row :gutter="20" class="expression-controls">
          <el-col :span="10">
            <el-select v-model="selectedLabel" placeholder="选择测点标签" clearable   style="--el-input-text-color: white">
              <el-option
          ...
          <el-col :span="11">
            <el-select v-model="selectedOperator" placeholder="选择运算符" clearable   style="--el-input-text-color: white">
