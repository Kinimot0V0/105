...
const getRealPointData = async () => {
  try {
    ...
    // [修改点] 过滤掉没有 turbineId 的条目，并提取唯一值
    const turbineIds = [...new Set(
      points
        .map(point => point.turbineId)
        .filter(id => id != null) // 过滤 null/undefined
    )];

    const uncachedIds = turbineIds.filter(id => !turbineCache.value[id]);

    if (uncachedIds.length > 0) {
      const turbinePromises = uncachedIds.map(id => 
        getFarmInfo({ turbineId: id })
          .then(res => res.data.result)
          .catch(error => {
            console.error(`获取风机${id}信息失败:`, error);
            return null;
          })
      );
      const turbineResults = await Promise.all(turbinePromises);

      turbineResults.forEach(info => {
        if (info) {
          turbineCache.value[info.turbineId] = {
            turbineName: info.turbineName,
            windFarmName: info.windFarmName
          };
        }
      });
    }

    // [修改点] 合并数据时处理可能缺失 turbineId 的情况
    realPointList.value = points.map(point => ({
      ...point,
      ...(point.turbineId ? turbineCache.value[point.turbineId] : {}) // 安全解构
    }));
