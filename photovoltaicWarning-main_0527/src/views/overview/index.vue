...
import { getPvFarmList, getDeviceWarnList,getFarmInfo,getDevice } from '@/api/overview'
...
const getWarningData = async () => {
  try {
    ...
    // 获取场站名并拼接
    // 提取所有唯一deviceId
    // const deviceIds = [...new Set(warningList.value.map(item => item.deviceId))];
    
    // // 并发获取场站信息
    // const farmRequests = deviceIds.map(deviceId => 
    //   getFarmInfo({ deviceId: deviceId })
    // )
    // const farmResponses = await Promise.all(farmRequests)

    // // 构建deviceId到场站名的映射
    // const deviceToFarmMap = {}
    // farmResponses.forEach((res, index) => {
    //   const deviceId = deviceIds[index]
    //   if (res.data && res.data.result) {
    //     deviceToFarmMap[deviceId] = res.data.result.pvFarmName
    //   }
    // })

    // // 更新warningList添加farmInverterName
    // warningList.value = warningList.value.map(item => ({
    //   ...item,
    //   farmInverterName: `${deviceToFarmMap[item.deviceId]}${item.deviceName}`
    // }))
