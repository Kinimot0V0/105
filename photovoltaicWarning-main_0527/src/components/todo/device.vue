    ...
    <ul class="tree">
      <li v-for="station in props.treeData" :key="station.pvFarm_id" class="tree-node">
        <div class="tree-label">
          ...
          <span class="label">
            {{ station.label }}
            <span class="count-badge" v-if="calculateStationCount(station) > 0">{{ calculateStationCount(station) }}</span>
          </span>
        </div>

        <ul v-if="station.expanded && station.children?.length">
          <li v-for="inverter in station.children" :key="inverter.device_id" class="tree-node" style="margin-left: 20px;">
            <!-- 1. 逆变器为叶子节点 -->
            <div v-if="!inverter.children" class="tree-label" :class="{ 'selected-device': inverter.selected }" @click="handleDeviceClick(inverter, station)">
              <span class="label">
            ...
            <ul v-if="inverter.expanded && inverter.children?.length">
              <li v-for="box in inverter.children" :key="box.device_id" class="child-node" :class="{ 'selected-device': box.selected }">
...
.selected-device .label {
  background: linear-gradient(rgba(119, 102, 30, 0) 0%, rgba(119, 102, 30, 0.5) 25%, rgba(119, 102, 30, 1) 50%, rgba(119, 102, 30, 0.5) 75%, rgba(119, 102, 30, 0) 100%);
}
.tree-label {
  display: flex;
  align-items: center;
