...
import { getInverterInfo,getTrendData,getFarmInfo,showPictures } from '@/api/warningDetail'
...
const getPointInfo = async () => {
  try {
    ...
    const response = await getInverterInfo(params);
    const result = response.data.result;
    const paramsFarm = { inverterId: props.inverterId };
    ...
    treeData.push({
      id: `${props.inverterId}`,
      label: `${pvFarmName.value}-${props.inverterName}`,
...
<template>
  <div class="container1">
      ...
      <div class="info-text">
        ...
        <span class="span-value" style="width: 220px;">{{ pvFarmName }}-{{ props.inverterName }}</span>
