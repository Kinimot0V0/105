...
/* ========= 3. 下拉列表含“全部” ========= */
const pvFarmListWithAll = computed(() => [
  { id: '', pvFarmName: '全部' },
  ...pvFarmList.value
])
const boxListWithAll = computed(() => [
  { boxId: '', boxName: '全部' },
  ...boxList.value
])
const inverterListWithAll = computed(() => [
  { inverterId: '', inverterName: '全部' },
  ...inverterList.value
])
const combinerListWithAll = computed(() => [
  { combinerId: '', combinerName: '全部' },
  ...combinerList.value
])
...
const getPvFarm = async () => {
  try {
    const { data } = await getPvFarmList()
...
<template>
  <div class="container">
    ...
    <el-table :data="warningList" style="margin-top: 10px">
      ...
      <el-table-column prop="inverterName"    label="逆变器名称" width="250" align="center" />
...
<style scoped>
.container   { padding:10px;margin-top:10px;border:1px solid #164b6d;height:80vh;display:flex;flex-direction:column; }
.company-line{ margin-bottom:10px; }
.filter-line { display:flex;flex-wrap:wrap;align-items:center;gap:15px;margin-bottom:10px; }
.label { display:flex;align-items:center;justify-content:center;background:#143f5c;border:1px solid #143f5c;
         font-size:15px;width:60px;height:8px;padding:10px; }
.selector        { width:160px; }
.level-selector  { width: 100px; }
.date-picker     { width:180px; }
.date-sep        { margin:0 5px;color:white; }
.company         { background:linear-gradient(to bottom,#102842,#15608a);width:140px;font-size:15px;
                   padding:15px;color:white;border:#164476; }
.pager-line { display:flex;justify-content:center;align-items:center;margin:20px 0;gap:10px;color:#c2dcf5; }
.span-name  { display:flex;align-items:center;justify-content:center;margin-left:15px;
              background:#143f5c;border:1px solid #143f5c;font-size:15px;width:60px;height:8px;padding:10px; }
.pv-farm-select { width:160px; }
.operation {
