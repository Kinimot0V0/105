...
import { getWindFarmList, getWarnList, getTurbineWarnList,getFarmInfo } from '@/api/overview'
...
const getWindFarm = async () => {
  try {
    ...
    // 在列表开头添加全部场站选项
    windFarmList.value = [
      { windFarmId: -1, windFarmName: '全部场站' },
...
const getWarningData = async () => {
  try {
    ...
    // 并发获取风场信息
    const farmRequests = turbineIds.map(turbineId => 
      getFarmInfo({ turbineId: turbineId })
    )
