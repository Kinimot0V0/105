...
const stepComponents = [StepOne, StepTwo]
...
const submitForm = async () => {
  const modelData = {
    ...
    pvfarmId: parseInt(stepData[0].pvFarmId),
    isAll: parseInt(stepData[0].isAll),
    deviceType: parseInt(stepData[0].deviceType),
    patternId: 1,
    algorithmId: parseInt(stepData[0].algorithmId),
    alertInterval: parseInt(stepData[0].alertInterval),
    alertWindowSize: parseInt(stepData[0].alertWindowSize),
    params: Object.fromEntries(Object.entries(stepData[0].params).map(([key, value]) => [key, parseInt(value, 10)])),
    deviceList: stepData[0].isAll === 1 ? [] : stepData[0].deviceList.map((device) => device.deviceId),
...
const beginTrain = async () => {
  const modelData = {
    ...
    deviceList: stepData[0].deviceList.map((device) => device.deviceId),
